...
using CSV, StatsPlots, LinearAlgebra, Random, Distributions
...
# ╔═╡ 5294dffc-7a3d-43b8-8b7b-b2a438c2737f
md"
### !!!IMPORTANT!!!
...
# ╔═╡ de7ccf5a-528b-4a12-bb0b-065d93eb5959
let
...
In this assignment, you will implement 2 different GLMs based on the derivations that we've done in class an in the notes.
As a warm-up---and a soft introduction to Julia---you will extend our implementation of ordinary linear regression with an L2 regularizer and with the ADAM optimizer.
Then you will implement your own multinomial logistic regression.

In each part of the assignment, look for 'TODO's in the certain cells.
These denote tasks that we expect you to complete.
"

# ╔═╡ 6d5b815d-fb8f-4dc7-9355-83f2e4e30c92
import LinearAlgebra: norm, norm1, norm2

# ╔═╡ 138a8468-21b6-4c59-9f88-33ca1d27b572
Random.seed!(1)
...
Throughout this assignment, you will first create a synthetic dataset using a known data-generation function, then you will implement a model that attempts to learn (or identify) what the data-generating function was.

The data generating function will take some inputs $X$ and transform them deterministically into some outputs $f(X)$. Then those outputs will be perturbed with random noise.
"

# ╔═╡ 28b5eb2b-2ea1-4aae-9bfe-efd55ae00e72
begin
	# Let's start with some random inputs
	
	samples = 30000
	features = 15
	
	# use a funky distribution to help detect bugs later on
	# if we use a Normal dist here, might hide some bugs
	X = rand(InverseGaussian(0.1, 1), samples, features)
...
md"
In the next cell, we need a linear function of the inputs. We leave it up to you to pick this linear function however you would like. A few restrictions:
...
# TODO: implement some linear function of the data
begin
	
	function f(X::AbstractMatrix)
		0.001*X[:,1]+0.005*X[:,2]+0.01*X[:,3]+0.0001X[:,4]
	end
end

# ╔═╡ e29cec5b-c04e-4073-9b2b-d1b228743da9
#### TEST CELL
let
	# is f(x) a linear function?
	a = rand(Uniform(-10, 10), 100, features)
	b = rand(Normal(2, 3), 100, features)
	
	@assert isclose(f(a) + f(b), f(a + b)) "f(x) does not satisfy f(a) + f(b) == f(a + b). Not a linear function."
	
	c = rand(Uniform(-5, 5))
	@assert isclose(c .* f(a), f(c .* a)) "f(x) does not satisfy c*f(a) == f(c*a). Not a linear function."
	
	"Good job. You did the thing"
end

# ╔═╡ 1c20e0e2-b491-4bd4-bfd3-d007c101dc3a
# TODO: Generate randomly perturbed outputs. That is Y ~ TODO(f(x))
# use a variance of >= 1
begin
	Random.seed!(1)
	Y = f(X) + rand(MvNormal(f(X),1))
end
...
density(Y, color=:black)

# ╔═╡ ec300f7e-0d83-48ea-8cf8-55adb31780e2
md"
### Linear Models

Note each model must have a consistent API.
...
md"For now, we will naively split our data into a train/test split"
...
md"
#### Ridge Regression [15pts]
"

# ╔═╡ cf388af0-3d04-41a2-b009-245356c78e63
md"
In this section, you will implement linear regression with l2 regularization (also known as ridge regression).
Modify the update rule for the linear regression solution given above to add l2 regularization.

For further (optional) reading, see section 6.2 in the notes!
"

# ╔═╡ 1e9a6ebc-8e79-47cd-ad82-de114232f97a
begin
	# We will get you started with the RidgeModel interface.
	
	# what should a RidgeModel object look like?
	struct RidgeModel <: Model
		W::Matrix
		lambda::Number
	end
	
	# How do we construct one?
	RidgeModel(in::Int, out::Int, lambda::Number) = RidgeModel(
		zeros(in, out), 
		lambda	
	)
	
	# How do we use it to make predictions?
	predict(m::RidgeModel, x::AbstractVector) = x'*m.W
	predict(m::RidgeModel, X::AbstractMatrix) = X*m.W
end
...
The function $f(X)$ defines a relationship between $y$ and $X$. Our linear models find an approximation $\hat{f}(X)$ which best fits noisy observations of $X$ and $y$. Adding regularization encodes some prior knowledge of the form of $f$, resticting the possible $\hat{f}$ which our linear model might find.

Go back to your simulated data generator and modify $f(X)$ so that ridge regression finds a better approximation than vanilla linear regression. Specifically, define a function $f(x)$ such that
```
RMSE(predict(lm, testX), testY) > RMSE(predict(rm, testX), testY)
```

Hint: think about what restrictions l2 regularization place on the approximation.
"

# ╔═╡ 87225565-d57c-4829-9e22-aabcdd59e571
md"
## ADAM Optimizer [15pts]
"
...
The original ADAM paper can be found [here](https://arxiv.org/pdf/1412.6980.pdf).
"
...
# TODO: fill in this struct with the needed hyperparameters
begin
	...
	ADAM(α, β1, β2, in, out) =
		ADAM(α, β1, β2, 1e-8, zeros(in, out), zeros(in, out))
end
...
As with the OLS implementation, we will start by constructing a synthetic dataset.
...
md"
### Synthetic Data [10pts]

We will also keep the same data matrix `X`. 
However, we have different probabilistic assumptions on our response matrix, `Y`, which we need our synthetic dataset to meet.
We will be doing 4-class classification, so our response matrix, `Y`, needs to be of shape `(samples, 4)`.

Implement a function `g(x)` which produces a vector ``y \in [0,1]^4`` for each sample in `X`.
A few limitations:
1. `g(x)` needs to depend on at least 2 columns of `X`
2. `g(x)` cannot be a constant function
3. ``y \notin \{0, 1\}^4``, that is `g(x)` should not produce a one-hot vector
4. The class balance in the dataset needs to be roughly even (at least 3000 of each class present)
"

# ╔═╡ 5b3774c9-2d8f-4a91-a258-45de3754fefa
# TODO: implement synthetic data func
begin
	Random.seed!(1)
	
	function g(X::AbstractMatrix)
		Y = Matrix{Float64}(undef, samples, 4)
		Y[:,1] = 2*X[:,1] + X[:,5] + 2*X[:,2]
		Y[:,2] = 2*X[:,1] + 2*X[:,5] + X[:,7]
		Y[:,3] = X[:,1] + 3*X[:,5] + X[:,12]
		Y[:,4] = 2*X[:,1] + X[:,5] + 2*X[:,15]
...
md"
Now we need to sample a response matrix `Y` according to the probabilities produced by `g(x)`.
"

# ╔═╡ a5acde42-39bf-4ede-8139-71b2b6dea178
begin
	Random.seed!(1)
	glmY = zeros(samples, 4)
	probs = g(X)
	for idx in 1:samples
		# TODO: glmY[idx, :] = ...
		sampleMN = rand(Multinomial(1,probs[idx,:]))
		glmY[idx, :] .= sampleMN
	end
	glmY
end

# ╔═╡ 17344fbb-408c-49a0-aa11-fc80eb670542
### TEST CELL
let
	@assert all(sum(glmY, dims=1) .> 3000) "Expected at least 3000 of each class to be present"
	
	@assert all(sum(probs, dims=2) .≈ 1) "Expected y = g(x) to sum to 1 for all values of x"
	
	@assert all(probs .!= 1) "Expected none of y to be a one-hot vector"
end
...
# ╔═╡ 2dc8bf97-cffd-43ab-9001-8b8499f1c08b
softmax(z::AbstractMatrix) = exp.(z)./sum(exp.(z))

# ╔═╡ 92f32acf-9ad8-438a-b289-d46e1f97b17e
# TODO: implement the struct, a constructor that defaults to weights=0, and the predict function
begin
	struct MultinomialLogisticModel <: Model
		W::Matrix
	end
	
	MultinomialLogisticModel(in::Int, out::Int) = MultinomialLogisticModel(
		zeros(in,out)
	)
	
	predict(m::MultinomialLogisticModel, x::AbstractVector) = softmax(x'*m.W)
	predict(m::MultinomialLogisticModel, X::AbstractMatrix) = softmax(X*m.W)
	
end

# ╔═╡ 9283d80e-c759-4f76-86c2-87bba8827777
begin
...
function runLinearModel(X::AbstractMatrix, Y::AbstractMatrix)
	...
	model = LinearModel(features, 1)

	# Each call to update! performs a single epoch
	for _ in 1:100
		train!(model, sgd, X, Y)
	end
...
lm = runLinearModel(trainX, trainY)

# ╔═╡ 4eca71f9-9342-4ed5-9ca8-ad3e39384d67
function runRidgeModel(X::AbstractMatrix, Y::AbstractMatrix)
	# train
	sgd = SGD(0.01)
	
	model = RidgeModel(features, 1, 1.)
...
rm = runRidgeModel(trainX, trainY)
...
function runLinearModelWithAdam(X::AbstractMatrix, Y::AbstractMatrix)
	...
	adam = ADAM(0.01, 0.9, 0.999, features, 1)
	model = LinearModel(features, 1)
...
lm_adam = runLinearModelWithAdam(trainX, trainY)

# ╔═╡ 141ce0e2-b2c3-4769-a67a-3c58311cd85d
RMSE(predict(lm, testX), testY)

# ╔═╡ 7b85d4d2-f405-448b-be9e-b2df4ffbf4bb
# TODO: implement the update func
begin
	function update!(lm::RidgeModel, sgd::SGD, x::AbstractVector, y::AbstractVector)
		y_hat = predict(lm, x)
		
		err = y_hat - y
		grad_W = err .* x + lm.lambda * lm.W
		lm.W .-= sgd.α * grad_W
	end
end

# ╔═╡ 7036ad67-4987-46fd-a9d5-f1ad2760f3f3
RMSE(predict(rm, testX), testY)

# ╔═╡ d3423d7c-7c78-4c6e-b09e-aee11fc82f2d
begin
	@assert RMSE(predict(lm, testX), testY) > RMSE(predict(rm, testX), testY) "The data is best fit using OLS. Keep modifying f(X) until the data is best fit with ridge regression"
...
begin
	function update!(lm::LinearModel, opt::ADAM, x::AbstractVector, y::AbstractVector)
		y_hat = predict(lm, x)
		err = y_hat - y' #added transpose to mlm
		ΔW = err .* x
		
		opt.M .= opt.β1*opt.M .+ (1-opt.β1)*ΔW
		opt.V .= opt.β2*opt.V .+ (1-opt.β2)*(ΔW).^2
		
		lm.W .-= opt.α*(opt.M ./ (sqrt.(opt.V) .+ opt.ϵ))
		
	end
end

# ╔═╡ d105a599-063c-4ee1-92e6-c12880ca77a2
RMSE(predict(lm_adam, testX), testY)

# ╔═╡ 49e540c5-a621-48db-94a2-b90b1f3e4ea1
begin
	@assert isclose(RMSE(predict(lm, testX), testY), RMSE(predict(lm_adam, testX), testY), 1.0) "We would have expected the LinearModel + ADAM to perform similarly to LinearModel + SGD. Try again!"
...
begin
	function update!(mlm::MultinomialLogisticModel, opt::ADAM, x::Vector, y::Vector)
		...
		mlm.W .-= opt.α*(opt.M ./ (sqrt.(opt.V) .+ opt.ϵ))
...
begin
	...
	function runMLM(X::AbstractMatrix, Y::AbstractMatrix)
		adam = ADAM(0.001, 0.9, 0.999, features, 4)

		model = MultinomialLogisticModel(features, 4)
...
mlm_model = runMLM(trainX, glm_trainY)

# ╔═╡ e884510d-36b6-49dc-ab89-335012133ecc
RMSE(predict(mlm_model, testX), glm_testY)

# ╔═╡ 0bb94dff-b436-499a-91af-d51665ff107b
md"
Note that we could also try to learn an OLS model of this data as well.
Our probabilistic assumptions no longer make sense, so hopefully we shall see that the OLS model is significantly out-performed by the Logistic Regressor.
"

# ╔═╡ 92916419-3e63-422d-890e-2dce5cc37c6c
function runOLSForClassification(X::AbstractMatrix, Y::AbstractMatrix)
	adam = ADAM(0.001, 0.9, 0.999, features, 4)

	model = LinearModel(features, 4)
...
lm_class = runOLSForClassification(trainX, glm_trainY)

# ╔═╡ 6f3ec55a-754f-4b8b-8b79-f5640f0529bc
RMSE(predict(lm_class, testX), glm_testY)

# ╔═╡ a2faef80-7d6f-41ae-a36a-364f0160fd75
function classificationError(ŷ, y)
...
# ╔═╡ c0812a94-b172-4aeb-9f57-d7bd1640c9ee
classificationError(predict(lm_class, testX), glm_testY)

# ╔═╡ 05288f31-da96-4fe7-86e5-8a0b640b7780
classificationError(predict(mlm_model, testX), glm_testY)

# ╔═╡ bd3d2b56-3c65-4f47-ad11-a4a17785a91f
begin
	@assert classificationError(predict(lm_class, testX), glm_testY) > classificationError(predict(mlm_model, testX), glm_testY) "The classification error for the multinomial logistic regressor should be smaller than for the naive OLS implementation"
